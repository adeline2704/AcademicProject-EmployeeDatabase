...
a3Emp * copyList(a3Emp * headLL){
   ...
   while(current !=NULL){
      ...
      //get empid and num dependents
      newNode->empId = current->empId;
      newNode->numDependents = current->numDependents;
      //copy name attributes
      strcpy(newNode->fname, current->fname);
      strcpy(newNode->lname, current->lname);

      //allocate space for dependents
      newNode->dependents = malloc(sizeof(char*)* newNode->numDependents);
      for(int i = 0; i<newNode->numDependents; i++){
         newNode->dependents[i] = malloc(sizeof(char)*MAX_LENGTH);
         strcpy(newNode->dependents[i], current->dependents[i]);
      }
      //adds employee to tail end of copied LL
